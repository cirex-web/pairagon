...
struct zDNA {
  ...
  char         *seqname;
	zSequence    *seq;
 	struct zHMM         *hmm; /* for getting isochore boundaries, not created or
								 freed by zDNA */
 	float         gc;         /* total gc level */
  	float        *gcs;        /* windowed gc level */
 	int          *tiso_group; /* windowed gc transition index */
 	int          *iiso_group; /* windowed gc state index */
	bool          complement;
...
void zInitDNA (zDNA*);
void zFreeDNA (zDNA*);
void zDNASetHMM(zDNA*,struct zHMM*);
...
struct zDNAIterator {
	zSequenceIterator* seq_iterator;
};
typedef struct zDNAIterator zDNAIterator;
