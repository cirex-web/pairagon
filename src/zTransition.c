...
static void zExplicitTrans (zTrellis *trellis, int from_state, int state, coor_t i) 
{
	...
	iso_group = zGetDurationIsochoreGroup(group, zGetDNAWindowedGC(trellis->dna,i));
						...
						int di, int subtrace, int sublen, score_t subscore, zDistribution *d)
{
	...
	tscore = zGetLocalTransitionScore(trellis, from_state, state,i);
	...
	if (from_state == subtrace)
		{
			...
			tscore = zGetLocalTransitionScore(trellis, subtrace, state,i);
...
score_t zScoreInternalState(zTrellis *trellis, int state, 
	...
	if (first_base) {
		...
		total_score += 
			zScoreDurationGroup(dg, 1, zGetDNAWindowedGC(trellis->dna,pos));
		...
		/*EVAN this else clause doesn't make any sense, it should probably be removed */
		if(flag == -1){
			total_score += 2000;
		}
	}
...
static void zGInternalTrans (zTrellis *trellis, int from_state, 
							 int state, coor_t i) {
	zInternalTransHelper(trellis, from_state, state, i, zGetLocalIntergenicContinueScore(trellis,i) );
}

static void zInternalTrans (zTrellis *trellis, int from_state, 
							int state, coor_t i) {
	zInternalTransHelper(trellis, from_state, state, i, zGetLocalTransitionScore(trellis,from_state,state,i));
}

score_t zScoreExplicitState(zTrellis *trellis, int state, 
							coor_t pos, coor_t length){
	...
	len_score = zScoreDurationGroup(group, length, zGetDNAWindowedGC(trellis->dna,pos));
	scan_score = zScoreScanners(trellis, name, state_name, pos-length-1,
										pos, ps, &do_return, &flag);
	
	if (do_return) return MIN_SCORE;
	
	return len_score+scan_score;
}
...
score_t zScoreExternalState(zTrellis *trellis, int ext_state, 
	...
	dscore = zScoreDurationGroup(hmm->dmap[hmm->state[ext_state].duration], 
								 length, zGetDNAWindowedGC(dna,pos));	
...
static void zExternalTrans (zTrellis *trellis, int int_state, int ext_state, coor_t pos){
	...
	*/
	...
	while(f != NULL){
		...
		total_score += zGetLocalTransitionScore(trellis, pre_state, ext_state,pos);
...
void zInternalTransBack(zTrellis *trellis, int from_state, 
	...
	total_score = scanner->score(scanner, (ps) ? i : trellis->dna->length-i-1)
	  + zGetLocalTransitionScore(trellis, from_state, to_state,i)
...
*/
	...
	t1score   = zGetLocalTransitionScore( trellis, int_state, ext_state,pos);
	...
	for (j = 0; j < sfv->size; j++) {
	  ...
	  for (pre_state = 0; pre_state < hmm->states; pre_state++) {
		  t2score = zGetLocalTransitionScore(trellis, ext_state, pre_state,pos);
		...
		length     = f->end = f->start + 1;
		cscore     = f->score;
		dscore     = zScoreDurationGroup(hmm->dmap[hmm->state[ext_state].duration], length, zGetDNAWindowedGC(dna,pos));
		gscore     = zGetLocalTransitionScore(trellis, int_state, int_state,pos);
...
static zScanner* nas_plus = NULL;
static zScanner* nas_minus = NULL;

void zSetScanners(zScanner **s, zScanner **cs, zScanner **ncs,
                  zScanner **es, zScanner **as, zScanner **nas, 
