...
int zMapHMM (zHMM *hmm) {
	...
	hmm->ammap = zCalloc(hmm->feature_count, sizeof(zModel*), "zHMM: ammap");
...
int zReadHMM (FILE *stream, zHMM *hmm, hmm_mode_t hmm_mode) {
	...
	if (phylo_models > 0) {
		hmm->phylo_model = zMalloc(phylo_models * sizeof(zModel), "zReadHMM phylo_models");
	}
