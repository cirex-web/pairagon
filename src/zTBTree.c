...
/* These are for checking the tree's integrity.  They were used during development
   and are kept in acse changes are made to this data structure and it again needs to
   be debugged 
...
void zReleaseDeadTBTreeNode(zTBTree* t, zTBTreeNode* n){
	...
	/* walk tree toward root removing new dead nodes */ 
 	while(n->parent->children == 1 && n->parent != t->root){
		n = n->parent;
