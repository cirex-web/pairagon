...
score_t zScoreInternalStateRange(zTrellis *trellis, int state, coor_t start, coor_t end){
	...
	score = zScoreInternalState(trellis,state,start,true);
 	...
 	for(i = start+1; i <= end; i++) {
		if(hmm->state[state].type == INTERNAL){
			tscore = zGetLocalTransitionScore(trellis,state,state,i);
		}
		else{
			tscore = zGetLocalIntergenicContinueScore(trellis,i);
		}
	}
...
score_t zGetLocalTransitionScore(zTrellis* trellis, int from_state, int to_state, coor_t pos){
	int iso_group= zGetDNATISOGroup(trellis->dna,pos);

	if (iso_group < 0 || iso_group >= trellis->hmm->iso_transitions ) {
		zDie("zGetLocalTransitionScore: iso_group %d not found from %d to %d\n", 
			 iso_group, from_state, to_state );
	}
	return trellis->hmm->tmap[from_state][to_state][iso_group];
}

score_t zGetLocalIntergenicContinueScore(zTrellis* trellis,coor_t pos) {
	int iso_group= zGetDNATISOGroup(trellis->dna,pos);
...
void zTracePartialTrellis (zTrellis *trellis, coor_t start, coor_t end, int state, zSFVec *sfv){
	...
	while (i > start) {
		...
		istate.score = trellis->cell[state][istate.end].score
			...
			- zGetLocalTransitionScore(trellis,trace,state,istate.start);
...
void zInitTrellis (zTrellis *trellis, char* dna_file, zHMM *hmm, char* conseq_file, 
	...
	zDNASetHMM(trellis->dna,trellis->hmm);
	...
	if(unmasked_dna_file != NULL){
		...
		zDNASetHMM(trellis->unmasked_dna,trellis->hmm);
		zCopyDNA(trellis->unmasked_dna, trellis->unmasked_rdna);
		zAntiDNA(trellis->unmasked_rdna);
		if(trellis->dna->length != trellis->unmasked_dna->length){
 			zDie("Unmasked DNA has different length than masked DNA\n");
 		}
	}
	else{
	...
	if(conseq_enabled) {
		...
		if(trellis->dna->length != trellis->conseq->length){
 			zDie("Conservation sequence has different length than DNA\n");
 		}
	}
	if(estseq_enabled) {
		...
		if(trellis->dna->length != trellis->estseq->length){
 			zDie("EST sequence has different length than DNA\n");
 		}
	}
	if(phylo_enabled) {
	...
	for (i = 0; i < hmm->feature_count; i++) {
		if (est_para_m) {
			...
			if(hmm->emmap == NULL){
				zDie("Parameter file must specify EST models if an estseq is used");
			}
	...
	if (phylo_enabled) {
		for (i = 0; i < hmm->feature_count; i++) {
			...
			trellis->alignmentscanner[i] = zMalloc(sizeof(zScanner), 
												   "zInitTrellis alignmentscanner");
			zInitAlignmentScanner(trellis->alignmentscanner[i], trellis->dna,
								  trellis->alignment, hmm->ammap[i]);
			trellis->ralignmentscanner[i] = zMalloc(sizeof(zScanner), 
													"zInitTrellis ralignmentscanner");
			zInitAlignmentScanner(trellis->ralignmentscanner[i], trellis->rdna,
								  trellis->ralignment, hmm->ammap[i]);

			
			zPreComputeScanner(trellis->alignmentscanner[i]);
			zPreComputeScanner(trellis->ralignmentscanner[i]);

			/* Clear scanner's shared cache */
			if(trellis->alignmentscanner[i]->uscore_block_count == 1){
				zBNTreeModelFlushCache(trellis->alignmentscanner[i]->model);
				/*zBNTreeModelFlushCache(trellis->ralignmentscanner[i]->model);*/
			}
		}
	}	
...
void zFreeTrellis (zTrellis *trellis) {
	...
	for (i = 0; i < trellis->hmm->feature_count; i++) {
		...
		if (trellis->alignment) {
			if (trellis->alignmentscanner[i] != NULL) {
				/* Clear scanner's shared cache */
				if(trellis->alignmentscanner[i]->uscore_block_count > 1){
					zBNTreeModelFlushCache(trellis->alignmentscanner[i]->model);
				}
				zFreeScanner(trellis->alignmentscanner[i]);
				zFree(trellis->alignmentscanner[i]);
				zFreeScanner(trellis->ralignmentscanner[i]);
				zFree(trellis->ralignmentscanner[i]);
			}
		}
	...
	if (trellis->alignment != NULL){
		zFree(trellis->alignmentscanner);  trellis->alignmentscanner = NULL;
		zFree(trellis->ralignmentscanner); trellis->ralignmentscanner = NULL;
 		zFreeAlignment(trellis->alignment);
 		zFreeAlignment(trellis->ralignment);
 		zFree(trellis->alignment);
 		zFree(trellis->ralignment);
 	}
...
zSFVec* zRunViterbiAndForward (zTrellis *trellis, score_t* path_score) {
	...
	zFreeSFList(sfl);
	zFree(sfl);
	zTrace2("terminating induction\n");
  for(j = 0; j < hmm->states; j++) 
    trellis->cell[j][trellis->dna->length - 1 - PADDING].score +=
      zGetInitProb(trellis->hmm, j, trellis->iiso_group);
...
void zRunBackward(zTrellis *trellis) {
	...
	for (i = length - PADDING - 1; i > 1; i--) {
		for (j = 0; j < hmm->states; j++) {
			...
			for (k = 0; k < hmm->states; k++) {
				if (MIN_SCORE == zGetLocalTransitionScore(trellis, j, k,i)) continue;
...
/* EVAN this doesn't quite work, but if someone fixes it it will be much more
   efficient then the implementation below
   the external score are off for some reason also this still needs to factor
   in the transition score also */ 
score_t zGetPathScore(zTrellis* trellis, const zSFVec* sfv, zSFVec* result) {
	score_t score = 0;
	int i;
	for(i = 0; i < sfv->size; i++){
		zSfeature f = sfv->elem[i];
		if(trellis->hmm->state[f.state].type == INTERNAL ||
		   trellis->hmm->state[f.state].type == GINTERNAL ){
			
			f.score = zScoreInternalStateRange(trellis,f.state,f.start,f.end);
		}
		else if(trellis->hmm->state[f.state].type == EXTERNAL){
			int feature = trellis->hmm->state[f.state].model;
			zFeatureFactory *ff = (f.strand == '-') ?
				trellis->factory[feature] : trellis->rfactory[feature];
			
			f.score = ff->scoref(ff,&f);
			
		}
		else{
			zDie("zGetPathScore does not support states other than INTERNAL and EXTERNAL");
		}
		score += f.score;
...
score_t zGetPathScoreOLD(zTrellis* trellis, const zSFVec* sfv, zSFVec* result) {
...
/**********************************************************\
  Pin Data Search Code                                   
	EVAN - below is an alternate pin search implementation
This was the first successful algorithm we used
The zRunPinViterbi implementation above is significantly
more efficient but I'll leave this here for now in case anyone
cares about it.  Feel free to remove it if you prefer.
\**********************************************************/
		...
		if(hmm->state[state].type == INTERNAL ||
		   hmm->state[state].type == GINTERNAL){
			...
			for(i = 0; i < jumps->size; i++) {
				...
				if(hmm->state[state].type == INTERNAL){
					score += zGetLocalTransitionScore(ps->trellis,prestate,state,ps->trellis->tiso_group);
				}
				else{
		...
		for(i = 0; i < jumps->size; i++) {
			...
			if(hmm->state[poststate].type == INTERNAL ||
			   hmm->state[poststate].type == GINTERNAL){
				...
				if(hmm->state[poststate].type == INTERNAL){
					score += zGetLocalTransitionScore(ps->trellis,state,poststate,ps->trellis->tiso_group);
				}
				else{
					score += zGetIntergenicContinueScore(ps->trellis);
				}
			...
			for(j = 0; j < prejumps->size;j++){
				...
				for(k = 0; k < postjumps->size;k++){
					...
					base_score = zScoreInternalState(trellis,poststate,f->end+1,true) +
						zGetLocalTransitionScore(ps->trellis,state,poststate,ps->trellis->tiso_group)+
						zScoreExternalState(trellis,state,f->end,f->end-f->start+1,f->score) + 
						zGetLocalTransitionScore(ps->trellis,prestate,state,ps->trellis->tiso_group);
		...
		for(i = 0;i < fmap->size;i++){
			...
			for(j = 0;j < jumps->size;j++){
				...
				base_score = zScoreExternalState(trellis,state,f->end,f->end-f->start+1,
												 f->score) +
					zGetLocalTransitionScore(trellis,prestate,state,trellis->tiso_group);
				from_pad = zListMoveFirst(&ps->live_from_list);
				while(from_pad != NULL){
