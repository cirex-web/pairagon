...
void zDNASetHMM(zDNA* dna,zHMM* hmm){
 	int i,j;
 	dna->hmm = hmm;
 	dna->tiso_group = zMalloc(sizeof(int)*dna->seq->block_count,"zDNASetHMM tiso_group");
 	dna->iiso_group = zMalloc(sizeof(int)*dna->seq->block_count,"zDNASetHMM iiso_group");
 	for(i = 0;i < dna->seq->block_count;i++){
 		for(j = 0;j < dna->hmm->iso_transitions;j++){
 			if(dna->gcs[i] < dna->hmm->iso_transition[j]){
 				dna->tiso_group[i] = j;
 				break;
 			}
 		}
 		for(j = 0;j < dna->hmm->iso_states;j++){
 			if(dna->gcs[i] < dna->hmm->iso_state[j]){
 				dna->iiso_group[i] = j;
 				break;
 			}
 		}
...
long zInitDNASequence(zSequence* seq, void *parent){
  ...
  dna->seqname = zGetSeqNameFromFastaHeader(dna->def);
...
int zReadDNASequence(zSequence* seq, zSeqBlock* block){
	...
	if(dna->hmm != NULL){
 		for(j = 0;j < dna->hmm->iso_transitions;j++){
 			if(dna->gcs[block->id] < dna->hmm->iso_transition[j]){
 				dna->tiso_group[block->id] = j;
 				break;
 			}
 		}
 		for(j = 0;j < dna->hmm->iso_states;j++){
 			if(dna->gcs[block->id] < dna->hmm->iso_state[j]){
 				dna->iiso_group[block->id] = j;
 				break;
 			}
 		}
 	}
...
void zFreeDNA(zDNA *dna) {
	...
	if(dna->gcs != NULL) {
 	...
 	if(dna->hmm != NULL){
 		zFree(dna->tiso_group);
 		zFree(dna->iiso_group);
	}
	if(dna->def != NULL){
		zFree(dna->def);
		dna->def = NULL;
...
void zCopyDNA(zDNA *dna, zDNA *copy) {
	...
	memcpy(copy->gcs,dna->gcs,sizeof(float)*dna->seq->block_count);
 	if(dna->hmm != NULL){
 		copy->hmm = dna->hmm;
 		copy->tiso_group = zMalloc(sizeof(float)*dna->seq->block_count,"zCopyDNA tiso_group");
 		copy->iiso_group = zMalloc(sizeof(float)*dna->seq->block_count,"zCopyDNA iiso_group");
 		memcpy(copy->tiso_group,dna->tiso_group,sizeof(int)*dna->seq->block_count);
 		memcpy(copy->iiso_group,dna->iiso_group,sizeof(int)*dna->seq->block_count);
 	}
...
float zGetDNAWindowedGC(zDNA* dna, coor_t pos){
	...
	/* didn't find the block, try to load it */
	if(i == -1){
		zGetSequencePos(dna->seq,pos);
		i = zGetSeqBlockID(dna->seq,pos);
	}
	if(i == -1){
		/* still didn't find it, pos must be in the padding */
		return dna->gc; 
	}
	else{
		return dna->gcs[i];
...
int zGetDNATISOGroup(zDNA* dna, coor_t pos){
 	int i = zGetSeqBlockID(dna->seq,pos);
 	if(i == -1){
 		zGetSequencePos(dna->seq,pos);
 		i = zGetSeqBlockID(dna->seq,pos);
	...
	if(i == -1){
		/* must be in the padding, doesn't matter what we return */ 
		return 0;
	}
	else{
		return dna->tiso_group[i];
	}
}

int zGetDNAIISOGroup(zDNA* dna, coor_t pos){
 	int i = zGetSeqBlockID(dna->seq,pos);
 	if(i == -1){
 		zGetSequencePos(dna->seq,pos);
 		i = zGetSeqBlockID(dna->seq,pos);
...
zDNAIterator* zGetDNAIterator(char* filename, char* snp_filename){
	zSequenceIterator* seq = zGetSequenceIterator(filename, 
					zInitGenericDNA, sizeof(zDNA), 
					zInitDNASequence, zReadDNASequence, zCreateDNASeqBlock,
					BLOCK_SIZE, BLOCK_COUNT);
	zDNAIterator*      dna = (zDNAIterator*) zMalloc(sizeof(zDNAIterator), "zGetDNAIterator: dna");
	dna->seq_iterator      = seq;
	if(snp_filename != NULL){
		zDie("Sequence variation cannot be handled for multifasta files yet");
	}
	return dna;
...
zDNA* zGetNextDNA(zDNAIterator* iterator) {
	return zGetNextParent(iterator->seq_iterator);
}
