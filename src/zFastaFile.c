...
int zReadFastaFile (FILE *stream, zFastaFile* entry){
  ...
  entry->seqname = zGetSeqNameFromFastaHeader(entry->def);
...
/* Retrieve the part of the header between the '>' and the first space 
 * Result is malloced, caller must free.  */
char * zGetSeqNameFromFastaHeader(const char *header)
{
  int s,e,len,i;
  char *seqname;
  ...
  for(e = 0; e < len && !isspace(header[e]); e++);

  if(e-s <= 0)
    zDie("Fasta header is improperly formatted.");

  seqname = zMalloc(e-s+1, "zGetSeqNameFromFastaHeader");
  for(i = s; i < e; i++)
    seqname[i-s] = header[i];
  seqname[e-s] = '\0';
