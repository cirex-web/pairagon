...
void* zCreateSeqBlockWithSize(coor_t block_size){
	...
	block->id = -1;
	block->map_idx = -1;
	block->data = zMalloc(block->size*sizeof(char),"zCreateSeqBlock block->data");
	return (void*)block;
}
...
static zSeqBlock* zLoadSeq(zSequence* seq, coor_t pos){
	...
	if(i == seq->map_size){
		zDie("zLoadSeq can't load past end of sequence: %d > %d\n",
			 pos,seq->length); 
	}
...
long zInitSequenceSpecialized (char* filename, void* parent,
	...
	if (fpos_end != -1) {
		seq->map_size = (fpos_end - fpos_start + 1)/seq->block_size + 2;
	} else {
		seq->map_size = stbuf.st_size/seq->block_size + 2;
	}
...
zSequenceIterator* zGetSequenceIterator (char* filename, 
					zGenericInitFunc init_parent_func,
					size_t parent_size,
					zSequenceInitFunc init_sequence_func, 
					zSequenceReadFunc read_func,
	...
	iterator = (zSequenceIterator*) zMalloc(sizeof(zSequenceIterator), "zGetSequenceIterator: iterator");
	iterator->filename = filename;
	iterator->parent   = NULL;
	iterator->parent_size = parent_size;
	iterator->init_parent_func = init_parent_func;
	iterator->init_sequence_func = init_sequence_func;
	iterator->read_func = read_func;
	iterator->init_block_func = init_block_func;
	iterator->block_size = block_size;
	iterator->block_count = block_count;
	iterator->fpos = 0;
	return iterator;
}

void* zGetNextParent(zSequenceIterator* iterator) {

	iterator->parent = zMalloc(iterator->parent_size, "zGetNextSequence: parent");
	iterator->init_parent_func(iterator->parent);
	iterator->fpos =  zInitSequenceSpecialized(iterator->filename,
								iterator->parent,
								iterator->init_sequence_func,
								iterator->read_func,
								iterator->init_block_func,
								iterator->block_size,
								iterator->block_count,
								iterator->fpos, 
								-1);
	if (iterator->fpos == 0) { /* EOF */
		return NULL;
	}
...
void zReverseSequence (zSequence *seq) {
	...
	for(i = 0;i < seq->var_count/2;i++){
 		var = seq->variants[seq->var_count-1-i];
 		seq->variants[seq->var_count-1-i] = seq->variants[i];
 		seq->variants[i] = var;
 	}
}
...
void zLoadSeqVariants(zSequence *seq,char *filename){
	...
	for(next_var = 0;next_var < var_count; next_var++){
		...
		if(seq->variants[i]->real_pos < last_pos){
			zDie("Bad sequence variant file: out of order (%d < %d)",seq->variants[i]->real_pos,last_pos);
		}
		...
		if(c == 'N'){
			/*EVAN this is bad.  zSequence shouldn't know anything about N masking */
