...
#define BNTREE_CACHE_SIZE 30e6  /* 30 MB cache per node */
...
int zReadModel (FILE *stream, zModel *model) {
	...
	if (model->type == WMM) {
		...
		if (MIX == model->type) {
			...
			/* check for GC data */
			unsigned int temp;
			(void)fscanf(stream, " GC_WINDOW=%u", &temp);
			model->window_size = (size_t)temp;
			mem = model->submodels;
			if (mem <= 0) {
...
/*BNTree specific functions */

int zReadBNTreeModel (FILE *stream, zModel *model) {
	int i;
	char type[64];
	char name[64];
	...
	if (model->type == BNTREE_ARRAY) {
		...
		model->submodel = zMalloc(length * sizeof(zModel), "zBNTreeModel allocate BNTREE_ARRAY submodels");
		...
		model->length = 1;
		model->submodels = 3;
		model->submodel = zMalloc(3 * sizeof(zModel), "zBNTree Model allocat BNTREE_CDS submodels");
		model->focus = order;
		/* read in all submodels */
		for (i=0; i<model->submodels; i++) {
			zReadBNTreeModel (stream, &(model->submodel[i]));
		...
		model->bntree = NewBNTreeFromFP (stream, name, order, BNTREE_CACHE_SIZE); 
	}

	return 1;
}

void zBNTreeModelFlushCache (zModel *model) {
	int i;
