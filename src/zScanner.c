...
#define NONCDS_BLOCK_SIZE 50000
#define ALIGNMENT_BLOCK_SIZE 50000
#define MIN_INCREMENTAL_ALIGNMENT_SCANNER_SIZE 5 /*1500000*/
/* never set this to 1, because it would cause huge ammounts of
   extra calculation and it is used as a special value for alignment scanners */
#define USCORE_BLOCK_COUNT 4
...
static score_t* zFillUScoreVariant(zScanner*,int,int,int,int,int);
...
static score_t zAlignmentScoreCDS (zScanner *scanner, zSfeature *f) {
	coor_t i;
	score_t score = 0;
	coor_t start, end;
	int frame = -1;
	...
	if (f->name == scanner->Einit || f->name == scanner->Esngl) {
		...
		start = end = 0;
		zDie("attempt to score non-CDS feature %s with BNTREE_CDS model", zStrIdx2Char(f->name));
	}
...
static score_t zBNTreeInfer (zModel *model, zDNA *dna,
							 zAlignment *alignment, coor_t pos) {
	int i, j;
	int tuple_idx;
	char tuple[256];
	double prob;
...
static score_t zScoreBNTREE (zScanner *scanner, coor_t pos) {

	if ((pos < scanner->min_pos) || (pos > scanner->max_pos)) {
		return MIN_SCORE;
	}

	return zBNTreeInfer (scanner->model, scanner->seq->parent, scanner->alignment, pos);
}

static score_t zScoreBNTREE_ARRAY (zScanner *scanner, coor_t pos) {
	int i;
	coor_t mfocus;
	double score = 0;
	zModel *this_submodel;
	...
	for (i=0; i<scanner->model->submodels; i++) {
		...
		score += zBNTreeInfer (this_submodel, scanner->seq->parent, scanner->alignment,
							   mfocus + i);
	}
...
static void zInitScannerReal(zScanner *scanner, zSequence *seq, 
	...
	/* this is only used for alignment scanners and currently
	   that is initialized in a separate function below */
	...
	if (model->type == WMM || model->type == LUT || model->type == WWAM || 
		...
		scanner->subscanners = model->submodels;
...
/* temporary init function until zAlignemnt can be folded into zSequence framework */
void zInitAlignmentScanner(zScanner *scanner, zDNA *dna, zAlignment *alignment, zModel *model){
	int i;
	...
	case BNTREE_CDS:
		...
		scanner->Esngl  = zChar2StrIdx("Esngl");
		scanner->Eterm  = zChar2StrIdx("Eterm");
		scanner->scoref = zAlignmentScoreCDS; 
		scanner->scorer = zIllegalScoreRange; 
		break;
	default:  
		scanner->scoref = zScoreFeature;
	...
	case BNTREE_CDS:
		...
		scanner->subscanner = zMalloc (3 * sizeof(zScanner), "zInitAlignmentScanner subscanners");
		for (i=0; i<3; i++){
			zInitAlignmentScanner (&scanner->subscanner[i], dna,
								   scanner->alignment, &scanner->model->submodel[i]);
			scanner->subscanner[i].parent = scanner;
		}
		break;
	default:
		scanner->subscanner = NULL;
...
void zFreeScanner(zScanner *scanner) {
	...
	for (i = 0; i < scanner->subscanners; i++)
...
static void zPreComputeScannerMap (zScanner* scanner){
	...
	/* get the block size */
	if(scanner->model->type == CDS || scanner->model->type == BNTREE_CDS){
		scanner->uscore_block_size = CDS_BLOCK_SIZE;
	}
	else{
		scanner->uscore_block_size = NONCDS_BLOCK_SIZE;
	}
	/*EVAN remove BNTREE_CDS if clause above if we keep this one*/
	if(scanner->alignment != NULL){
		scanner->uscore_block_size = ALIGNMENT_BLOCK_SIZE;
	}
	/* since the aligment scanner requires saving a bunch of stuff (see /bntree/ subdir)
	   if we are going to fill it incrementally we get better memory savings for small
	   sequences by just storing the whole thing and dumping the extra stuff */
	scanner->uscore_block_count = USCORE_BLOCK_COUNT;
	if(scanner->alignment != NULL &&
	   scanner->alignment->length <= MIN_INCREMENTAL_ALIGNMENT_SCANNER_SIZE){
		scanner->uscore_block_size = scanner->alignment->length+1;
		/* block_count of 1 signals the full sequence is in the scanner at once */
		scanner->uscore_block_count = 1;
	}
	...
	for(i = 0;i < scanner->uscore_map_size;i++){
		...
		scanner->uscore_map[i].loads = 0;
	}
	/* get pointers to sequence variants in each uscore_map */
...
void zPreComputeScanner(zScanner* scanner) {
	...
	if (ISO == scanner->model->type) {
	...
	else if (CDS == scanner->model->type || BNTREE_CDS == scanner->model->type){
		...
		if(ps){
			/* fill begining of sequence for + strand */
			...
			/* fill end of sequence for - strand */
			for(j = 0; j < zIntMin(scanner->uscore_block_count,scanner->uscore_map_size); j++){		
				zFillUScore(scanner,scanner->uscore_map_size-1-j,j);
				zPtrListAddLast(&scanner->uscore_list,&scanner->uscore[j]);
			}
		}
	} 
		...
		for(i = 0;(int)i < scanner->uscore_block_count;i++){
			...
			if(ps){
				...
				/* fill end of sequence for + strand */
				zFillUScore(scanner,scanner->uscore_map_size-1-i,i);
			}
...
static score_t zScoreRangeBlock(zScanner* scanner,zUScoreBlock* block, coor_t end, 
	...
	if(scanner->seq->var_count == 0){	
		...
		score = 0;
		while(end+1 > zCoorMax(start+1,block->pos)){
			...
			if(v == 0){
				...
				var = map->vars[v]; 
				var_scores = zFillUScoreVariant(scanner,block->map_idx,block->idx,v,1,-1);
...
/* return score for scanner (with frame if not -1) for start to end inclusive */
...
score_t zGetUScore(zScanner *scanner,coor_t i){
	...
	if(v <= 0){
		...
		if(scores == NULL){
			scores = zFillUScoreVariant(scanner,block->map_idx,block->idx,v,0,-1);
		}
...
static int zGetScannerVariantScoreIndex(zScannerVariant* var){
	...
	var_val = 0;
	for(i = 0; i < var->var_count; i++){
		var_val += var->vars[i]->cur * mul;
		mul *= var->vars[i]->variants;
	}
	return var_val;
}

static int zSetScannerVariantScoreIndex(zScannerVariant* var,int v){
	int i,var_val;
	int old_v = v;
	int mul = 1;
	var_val = 0;
	for(i = 0; i < var->var_count-1; i++){
		mul *= var->vars[i]->variants;
	}
	for(i = var->var_count-1; i > 0; i--){
		var->vars[i]->cur = (int)v/mul;
		v -= ((int)v/mul)*mul;
		mul /= var->vars[i-1]->variants;
	}
	var->vars[i]->cur = v;
	if(old_v != zGetScannerVariantScoreIndex(var)){
		zDie("Bad scanner variant fill");
	}
	return var_val;
}
...
static score_t* zFillUScoreVariant(zScanner* scanner,int map_idx,int block_idx,int var_idx,
	...
	if(v == -1){
		...
		zSetScannerVariantScoreIndex(var,v);
		zSetSequenceVariants(scanner->seq);
	}
	if(sum){
	...
	/* fill in the scores */
	if (CDS == scanner->model->type || BNTREE_CDS == scanner->model->type) {
		...
		var->score_sum[v][0] = var->score[v][0];
...
static void zFillUScore(zScanner *scanner,int map_idx,int block_idx){
	...
	if (ISO == scanner->model->type){
		...
		/* EVAN this is used for optimizing block sizes and counts.  If things are getting reloaded often we are wasting
		   work.  Ideally we would have 0 reloads, though averaging close to 0 is OK. */
		/*if(map->loads > 1){
			fprintf(stderr,"Reload%d %s%c %d\n",map->loads,scanner->model->name,scanner->seq->reverse?'-':'+',map->pos);
			}*/
		...
		if(scanner->seq->var_count > 0){
			...
			if(map->var_count > 0){
				...
				for(v = 0;v < map->var_count; v++){
					int vals;
					for(vals = 0; vals < map->vars[v]->var_vals; vals++){
						if(map->vars[v]->score[vals] == NULL){
							zFillUScoreVariant(scanner,map_idx,block_idx,v,0,vals);
						}
					}
					last = -1;
