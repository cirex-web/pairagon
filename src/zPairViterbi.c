...
void zInitAlleleDecodePair(zAlleleDecode* ad, size_t header_size, size_t val_size){
	ad->sfv = zMalloc(sizeof(zSFVec),"zInitAlleleDecodePair ad->sfv");
	zInitSFVec(ad->sfv,1);
	ad->sfl = zMalloc(sizeof(zSFList),"zInitAlleleDecodePair ad->sfl");
	zInitSFList(ad->sfl);

	ad->afv = zMalloc(sizeof(zAFVec),"zInitAlleleDecodePair ad->afv");
	zInitAFVec(ad->afv,1);
	ad->afl = zMalloc(sizeof(zAFList),"zInitAlleleDecodePair ad->afl");
	zInitAFList(ad->afl);

	ad->header = zMalloc(sizeof(char)*header_size,"zInitAlleleDecodePair ad->header");
	ad->vals = zMalloc(sizeof(short)*val_size,"zInitAlleleDecodePair ad->vals");
}

void zFreeAlleleDecodePair(zAlleleDecode* ad){
	if(ad->sfv != NULL){
		zFreeSFVec(ad->sfv);
		zFree(ad->sfv);
	}
...
static void zInitPairViterbi(zViterbi* v,zPairTrellis* trellis){
	...
	for(i = 0;i < v->max_trellis_count; i++){
		...
		v->tb[i] = zMalloc(sizeof(zViterbiTraceback),
								"zInitViterbi tb[i]");
...
static void zSNPSetSeqForAllelePair(zViterbi* v,int allele){
	int i,j,f;
...
static void zSNPUnSetSeqForAllelePair(zViterbi* v,int allele){
	int i,f;
	if(v->snp_count[allele] == 0){
		return;
	}
...
static int zSNPViterbiCreateLiveNodesPair(zViterbi* v,int allele){
	...
	zSNPViterbiCreateLiveNodesPair(v,allele);
...
static void zExtendState(zViterbi *viterbi, int allele, zPairTrellis *trellis, coor_t gpos, coor_t cpos, int from_state, int to_state) {
	score_t         score;
	zHMM           *hmm           = trellis->hmm;
	zTBTreeNode ****cache         = viterbi->tb[allele]->cache;
	zTBTree        *tree          = &viterbi->tb[allele]->tbtree;
...
void zRunSNPPairViterbiOnBlock(zViterbi* viterbi, zPairTrellis* trellis, zHSP* block){
	...
	for(viterbi->pos = MAX(block->g_start, trellis->blocks->gb_start); viterbi->pos <= MIN(block->g_end, genomic->length - trellis->padding - 1); viterbi->pos++){
		...
		for(allele = 0; allele < viterbi->trellis_count; allele++){
			...
			cache = viterbi->tb[allele]->cache;
			zSNPSetSeqForAllelePair(viterbi,allele); /* No effect */
			/* STEP 1 - create new live nodes */
/*
			zSNPViterbiCreateLiveNodesPair(viterbi,allele);
*/
			...
			*/
			...
			zSNPUnSetSeqForAllelePair(viterbi,allele);
		}
	}
}
...
zPtrList* zRunSNPPairViterbi(zPairTrellis* trellis, score_t *path_score){
	...
	*/
	...
	zFinishAlignmentForward(viterbi->tb[0]->cache[(viterbi->pos-1)%viterbi->cache_length], trellis, viterbi->cdna_pos - 1); /* viterbi->cdna_pos is 1 more than max */

	/* Viterbi trace back */
	zTrace2("traceback\n");
	*path_score = 0;

	cells = viterbi->tb[0]->cache[(viterbi->pos-1)%viterbi->cache_length][viterbi->cdna_pos - 1]; /* viterbi->cdna_pos is 1 more than max */
	best_state = -1;
	best_score = MIN_SCORE;
	for(state = 0;state < hmm->states;state++){
		if(cells[state] != NULL && 
	...
	*path_score = best_score;
	...
	zInitAlleleDecodePair(ad,25,1);
	ad->vals[0] = 0;
	
	sprintf(ad->header,"## Main SNP Sequence\n"); 
	
	zTBTree2AFL(ad->afl,&viterbi->tb[0]->tbtree,cells[best_state],hmm,trellis);
