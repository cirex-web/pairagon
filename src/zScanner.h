...
struct zUScoreMap {
	...
	int               loads;      /* used to debug/optimize block sizes */
};	
...
struct zScanner {
	...
	int               subscanners; /* number of subscanners */
	...
	int id;
	zAlignment      *alignment; /*hack until the zAlignment object in incorporated 
								  into the zSequence framework */ 
};
typedef struct zScanner zScanner;

void zFreeScanner (zScanner*);
void zInitScanner (zScanner*, zSequence*, zModel*);
void zInitAlignmentScanner (zScanner*, zDNA*, zAlignment*, zModel*);
