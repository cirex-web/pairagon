...
#define MINIMUM_EXON_SIGNAL_SCORE -100 /* -100 may be a litte bit high */
...
static score_t zScoreAlignmentCoding(const zFeatureFactory* fac, zSfeature *f){
	score_t score = 0;
	frame_t frame = -1;
	coor_t start,end;
	zScanner* scanner = fac->alignscan[CDSALIGN];
	...
	default: zDie("zScoreBNTREE_CDS:  invalid exon frame");
	}
	
	score = zGetRangeScore(scanner,end,start-1,frame); 
...
static score_t zScoreEsngl (const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		...
		if (fac->alignscan[CDSALIGN]->model->type == BNTREE) {
			/* no frame dependence */
			cdsalign_score = zScoreRange(fac->alignscan[CDSALIGN],exon->start+6,exon->end-3);
		}
		else if (fac->alignscan[CDSALIGN]->model->type == BNTREE_CDS) {
			/* frame dependence */
			cdsalign_score = zScoreAlignmentCoding(fac, exon);
		}
		cdsalign_score -= zScoreRange(fac->alignscan[INTRONALIGN],exon->start+6,exon->end-3);
		
		initalign_score = zGetUScore(fac->alignscan[INITALIGN],exon->start);
		initalign_score -= zScoreRange(fac->alignscan[INTRONALIGN],exon->start-6,exon->start+5);

		termalign_score = zGetUScore(fac->alignscan[TERMALIGN],exon->end-2);
		termalign_score -= zScoreRange(fac->alignscan[INTRONALIGN],exon->end-2,exon->end+3);
...
static void zMakeEpasFrom3 (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	if (zGetUScore(fac->scan[START],pos+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE) {
		...
		if (pos < PADDING + fac->max_size - 1){
			begin = PADDING;
		}
		else{
			begin = pos - fac->max_size + 1;
		}
...
static void zMakeEpasFrom5 (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	/* min size 6 */
	for(i = pos + 5; i < pos + fac->max_size; i++){
...
static void zMakeEpasFromInside (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	for(end = pos; end < pos + fac->max_size; end++){
		if (zGetUScore(fac->scan[START],end+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE) {
			begin = end - fac->max_size + 1;
			if(end  < PADDING + fac->max_size - 1){
				begin = PADDING;
			}
			else{
				begin = end - fac->max_size + 1;
			...
			/* min size 6 */
			for(;begin < zCoorMin(pos+1,end - 4); begin++){ 
				exon = zSFListAppend(sfl);			
				exon->group  = NULL;
				exon->strand = fac->strand;
				exon->name   = fac->type;
...
static void zMakeEpsFrom5 (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	/* min size 3 */
	for(i = pos + 2; i < pos + fac->max_size;i++){
...
static void zMakeEpsFromInside (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	for(end = pos; end < pos + fac->max_size; end++){
		if (zGetUScore(fac->scan[DON],end+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE) {
			...
			/* min size 2*/
			for(;begin < zCoorMin(pos+1,end); begin++){ 
				exon = zSFListAppend(sfl);			
				exon->group  = NULL;
				exon->strand = fac->strand;
...
static void zMakeEasFrom3 (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	if (zGetUScore(fac->scan[START],pos+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE) {
		...
		if (pos < (PADDING+40)  + fac->max_size - 1){
			begin = PADDING + 40;
		}
		else{
			begin = pos - fac->max_size + 1;			
...
static void zMakeEasFrom5 (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	if (zGetUScore(fac->scan[ACC],pos-1) > MINIMUM_TWINSCAN_SIGNAL_SCORE){
		/* min size 9 */
		for(i = pos + 8; i < pos + fac->max_size; i++){
			if(zGetUScore(fac->scan[START],i+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE){
				exon = zSFListAppend(sfl);			
				exon->group  = NULL;
				exon->strand = fac->strand;
...
static void zMakeEasFromInside (const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	for(end = pos; end < pos + fac->max_size; end++){
		if (zGetUScore(fac->scan[START],end+1) > MINIMUM_TWINSCAN_SIGNAL_SCORE) {
            /* 40 here allows enough space to score acceptor */
            /* model at the 5' end of the exon->              */
			if(end  < PADDING + 40 + fac->max_size - 1){
				begin = PADDING + 40;
			}
			else{
				begin = end - fac->max_size + 1;
			...
			/* min size 8*/
			for(;begin < zCoorMin(pos+1,end - 6); begin++){ 
...
static score_t zScoreEpa(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		align_score = zScoreRange(fac->alignscan[EPA_ALIGN],exon->start,exon->end-6) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start,exon->end - 6);		
...
static score_t zScoreEp(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		align_score = zScoreRange(fac->alignscan[EP_ALIGN],exon->start,exon->end-3) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start,exon->end-3);
		
		align_score += zGetUScore(fac->alignscan[UTRDONALIGN],exon->end+1) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->end-2,exon->end+6);
...
static score_t zScoreEa(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		align_score = zScoreRange(fac->alignscan[EA_ALIGN],exon->start+3,exon->end-6) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start+3,exon->end-6);
		
		align_score += zGetUScore(fac->alignscan[UTRACCALIGN],exon->start-1) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start-40,exon->start+2);
...
static score_t zScoreEnc(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		align_score = zScoreRange(fac->alignscan[ENC_ALIGN],exon->start+3,exon->end-3) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start+3,exon->end-3);
...
static score_t zScoreEterm(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		...
		if (fac->alignscan[CDSALIGN]->model->type == BNTREE) {
			/* no frame dependence */
			cdsalign_score = zScoreRange(fac->alignscan[CDSALIGN],exon->start+3,exon->end-3);
		}
		else if (fac->alignscan[CDSALIGN]->model->type == BNTREE_CDS) {
			/* frame dependence */
			cdsalign_score = zScoreAlignmentCoding(fac, exon);
		}
		cdsalign_score -= zScoreRange(fac->alignscan[INTRONALIGN],exon->start+3,exon->end-3);
		
		accalign_score = zGetUScore(fac->alignscan[ACCALIGN],exon->start-1) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start-40,exon->start+2);
		
		termalign_score = zGetUScore(fac->alignscan[TERMALIGN],exon->end-2) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->end-2,exon->end+3);
...
static score_t zScoreEinit(const zFeatureFactory *fac, zSfeature *exon) {
	...
	if (fac->phylo_enabled) {
		...
		initalign_score = zGetUScore(fac->alignscan[INITALIGN],exon->start) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->start-6,exon->start+5);

		donalign_score = zGetUScore(fac->alignscan[DONALIGN],exon->end+1) - 
			zScoreRange(fac->alignscan[INTRONALIGN],exon->end-2,exon->end+6);
...
static void zMakeExonsFrom3(const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	if (zGetUScore(fac->scan[DON],pos+1)  > MINIMUM_EXON_SIGNAL_SCORE) {	
		if(fac->estseq != NULL && !fac->est_para_mode){
			if(zGetEstseqSeq(fac->estseq,pos+1) == '1'){
				return;
			}
		...
		for (roh = 0; roh < 3; roh++) {
			...
			for (i = end; i >= begin; i--) { 
				if((i <= PADDING+43) || 
				   (zGetUScore(fac->scan[ACC],i-1) <= MINIMUM_EXON_SIGNAL_SCORE)) 
					continue;
				if(fac->estseq != NULL && !fac->est_para_mode){
					if(zGetEstseqSeq(fac->estseq,i-1) == '1'){
						continue;
...
static void zMakeExonsFrom5(const zFeatureFactory *fac, coor_t pos, zSFList* sfl) {
	...
	/* MINIMUM_TWINSCAN_SIGNAL_SCORE is for genscan compatibility */
	if ((pos > PADDING+43) && 
		(zGetUScore(fac->scan[ACC],pos-1) > MINIMUM_EXON_SIGNAL_SCORE)){	
		...
		for (loh = 0; loh < 3; loh++){		
			...
			for (i = begin; i <= end; i++) { 
				if (zGetUScore(fac->scan[DON],i+1) <= MINIMUM_EXON_SIGNAL_SCORE)
					continue;
				if(fac->estseq != NULL && !fac->est_para_mode){
					if(zGetEstseqSeq(fac->estseq,i+1) == '1'){
						continue;
...
void zInitEFactory(zFeatureFactory *factory, zDNA* dna, zEstseq* estseq, zScanner** scanners, zScanner**  conseqscanners, zScanner** estseqscanners, zScanner **alignmentscanners, int feature_count, zStrIdx type, zFFCreate create5, zFFCreate create3, zFFCreate createi, strand_t strand, bool conseq_enabled, bool est_para_mode,bool phylo_enabled, zStopSeq* stop_seq, zEstseqSearch* estsearch){
	...
	zScanner *cdsalign_scan;
	zScanner *intronalign_scan;
	zScanner *initalign_scan;
	zScanner *termalign_scan;
	zScanner *accalign_scan;
	zScanner *donalign_scan;
	zScanner *ep_align_scan;
	zScanner *enc_align_scan;
	zScanner *ea_align_scan;
	zScanner *epa_align_scan;
	zScanner *utraccalign_scan;
	zScanner *utrdonalign_scan;
	...
	if(phylo_enabled) {
		zPreComputeScanner(cdsalign_scan);
		zPreComputeScanner(intronalign_scan);
		zPreComputeScanner(initalign_scan);
		zPreComputeScanner(termalign_scan);
		zPreComputeScanner(accalign_scan);
		zPreComputeScanner(donalign_scan);
		if (utr_struct_enabled) {
			zPreComputeScanner(ep_align_scan);
			zPreComputeScanner(enc_align_scan);
			zPreComputeScanner(ea_align_scan);
			zPreComputeScanner(epa_align_scan);
			zPreComputeScanner(utraccalign_scan);
			zPreComputeScanner(utrdonalign_scan);
		}
	}
	...
	if(ISO == einit_cds_scan->model[0].type){
		factory->offset  = einit_cds_scan->subscanner[0].subscanner[0].model->focus;		
	}
	...
	factory->alignscan = zMalloc (NUM_EXON_ALIGNSCANNERS * sizeof (zScanner *),
